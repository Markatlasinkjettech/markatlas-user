  ...
  return (
    <header className="flex justify-between items-center p-4 bg-white fixed top-0 left-0 right-0 z-50 shadow-md">
      <div className="flex items-center animate-fadeInLeft">
        <span className="text-2xl font-bold">MarkAtlas Inkjet</span>
      </div>
        ...
        {links.map((link, index) => (
          link.name === 'Services' ? (
            ...
            <Link
              key={index}
              to={link.path}
              className={`flex items-center text-gray-600 hover:text-orange-500 transition-colors duration-300 ${link.name === 'Blog' ? 'text-orange-500' : ''}`}
            >
              {link.name}
            </Link>
          )
          ...
          {links.map((link, index) => (
            link.name === 'Services' ? (
              ...
              <Link
                key={index}
                to={link.path}
                className={`text-gray-600 hover:text-orange-500 transition-colors duration-300 text-lg ${link.name === 'Blog' ? 'text-orange-500' : ''}`}
                onClick={() => setIsMenuOpen(false)}
              >
                {link.name}
              </Link>
