        ...
        {links.map((link, index) => (
          link.name === 'Services' ? (
            ...
            <Link
              key={index}
              to={link.path}
              className={`flex items-center text-gray-600 hover:text-orange-500 transition-colors duration-300 ${link.name === 'Home' ? 'text-orange-500' : ''}`}
            >
              {link.name}
            </Link>
          )
          ...
          {[...links, { name: 'Contact Us', path: '/contact-us' }].map((link, index) => (
            <div key={index} className="w-full text-right">
              {link.name === 'Services' ? (
                ...
                <Link
                  to={link.path}
                  className={`block text-gray-600 hover:text-orange-500 transition-colors duration-300 text-lg ${link.name === 'Home' ? 'text-orange-500' : ''}`}
                  onClick={() => setIsMenuOpen(false)}
                >
                  {link.name}
                </Link>
