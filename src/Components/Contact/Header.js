...
const Header = () => {
  ...
  return (
    <header className="flex justify-between items-center p-4 bg-white fixed top-0 left-0 right-0 z-50 shadow-md">
        ...
        {links.map((link, index) => (
          link.name === 'Services' ? (
            ...
            <Link
              key={index}
              to={link.path}
              className={`flex items-center text-gray-600 hover:text-orange-500 transition-colors duration-300 ${link.name === 'Contact Us' ? 'text-orange-500' : ''}`}
            >
              {link.name}
            </Link>
          )
      ...
      <Link
        to="/contact-us"
        className="hidden md:flex items-center text-orange-500 hover:text-orange-500 transition-colors duration-300"
      >
        Contact Us
      </Link>
          ...
          {[...links, { name: 'Contact Us', path: '/contact-us' }].map((link, index) => (
            <div key={index} className="w-full text-right">
              {link.name === 'Services' ? (
                ...
                <Link
                  to={link.path}
                  className={`block text-gray-600 hover:text-orange-500 transition-colors duration-300 text-lg ${link.name === 'Contact Us' ? 'text-orange-500' : ''}`}
                  onClick={() => setIsMenuOpen(false)}
                >
                  {link.name}
                </Link>
