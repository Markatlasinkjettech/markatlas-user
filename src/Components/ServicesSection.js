    ...
    <div className="flex flex-col lg:flex-row justify-center items-stretch max-w-7xl mx-auto">
      ...
      <div className="lg:w-1/2 p-2 lg:p-4">
        {services.map((service, index) => (
          <motion.div
            key={index}
            className="bg-white rounded-lg shadow-lg p-4 lg:p-6 mb-4 lg:mb-6 transition-all duration-300 hover:shadow-xl border border-blue-400 relative overflow-hidden group"
            initial={{ opacity: 0, x: 50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5, delay: index * 0.2 }}
            whileHover={{ scale: 1.02 }}
          >
            <div className="absolute inset-0 bg-gradient-to-r from-orange-400 via-orange-500 to-orange-200 opacity-0 group-hover:opacity-20 transition-opacity duration-500 bg-[length:200%_100%] animate-gradient"></div>
            <h2 className="text-xl lg:text-2xl font-semibold text-blue-600 mb-2 lg:mb-3 relative z-10">{service.title}</h2>
            <p className="text-gray-700 mb-3 lg:mb-4 relative z-10">{service.description}</p>
            <motion.div
              className="flex items-center text-orange-500 font-semibold relative z-10"
              whileHover={{ x: 5 }}
            >
