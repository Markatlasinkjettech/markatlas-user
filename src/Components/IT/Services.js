    ...
    <div className="bg-soft-gray p-8">
          ...
          >
            ...
            >
              <div className="bg-blue-100 p-6 flex items-center justify-between">
                <h2 className="text-2xl font-semibold text-dark-gray">{services[currentIndex].title}</h2>
                <motion.div
                  animate={{ rotate: 360 }}
                  transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
                >
                  {CurrentIcon && <CurrentIcon size={32} className="text-light-green" />}
                </motion.div>
              </div>
              <div className="p-6">
                ...
                {services[currentIndex].details.map((item, idx) => (
                  ...
                  >
                    <div className="w-2 h-2 bg-dark-gray rounded-full mt-2 mr-3 flex-shrink-0" />
      ...
      <div className="flex justify-center mt-8 space-x-2">
        {services.map((_, index) => (
          <motion.button
            key={index}
            className={`w-3 h-3 rounded-full ${index === currentIndex ? 'bg-blue-700' : 'bg-blue-500'}`}
