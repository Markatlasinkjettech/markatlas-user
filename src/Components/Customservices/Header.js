      ...
      <nav className="hidden md:flex space-x-4 animate-fadeInRight">
        {links.map((link, index) => (
          link.name === 'Services' ? (
            ...
            >
              <button
                className="flex items-center text-orange-500 hover:text-orange-500 transition-colors duration-300"
              >
                {link.name} 
            ...
            <Link
              key={index}
              to={link.path}
              className={`flex items-center text-gray-600 hover:text-orange-500 transition-colors duration-300 ${link.name === 'Services' ? 'text-orange-500' : ''}`}
            >
              {link.name}
            </Link>
          )
          ...
          {[...links, { name: 'Contact Us', path: '/contact-us' }].map((link, index) => (
            <div key={index} className="w-full text-right">
              {link.name === 'Services' ? (
                ...
                <Link
                  to={link.path}
                  className={`block text-gray-600 hover:text-orange-500 transition-colors duration-300 text-lg ${link.name === 'Services' ? 'text-orange-500' : ''}`}
                  onClick={() => setIsMenuOpen(false)}
                >
                  {link.name}
                </Link>
